...
.. note::
    ...
    If you get **docker: Error response from daemon: Unknown runtime specified nvidia.** when running docker with ``--runtime=nvidia``.
    Please update your ``nvidia-docker`` to ``nvidia-docker2`` and replace ``--runtime=nvidia`` with ``--gpus all``.
