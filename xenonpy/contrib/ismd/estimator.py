...
class GaussianLogLikelihood(BaseLogLikelihood):
    def __init__(self, descriptor: Union[BaseFeaturizer, BaseDescriptor], *, targets={}, **estimators: BaseEstimator):
        ...
        if not isinstance(descriptor, (BaseFeaturizer, BaseDescriptor)):
            raise TypeError(
                '<descriptor> must be a subclass of <BaseFeaturizer> or <BaseDescriptor>')
    ...
    def __setitem__(self, key, value):
        if not (hasattr(value, 'predict') and isinstance(value.predict, MethodType)):
            raise TypeError(
                'estimator must be a regressor in scikit-learn style')
    ...
    def predict(self, smiles, **kwargs):
        fps = self._descriptor.reactant_transform(smiles, return_type='df')
        fps_ = fps.dropna()
        tmp = {}
        for k, v in self._mdl.items():
            if isinstance(v, BayesianRidge):
                tmp[k + ': mean'], tmp[k +
                                       ': std'] = v.predict(fps_, return_std=True)
            else:
    ...
    def fit(self, smiles, y=None, *, X_scaler=None, y_scaler=None, **kwargs):
        ...
        if not isinstance(y, (pd.DataFrame, pd.Series)):
            raise TypeError(
                'please package all properties into a pd.DataFrame or pd.Series')

        # remove NaN from X
        desc = self._descriptor.reactant_transform(
    ...
    def log_likelihood(self, smis, *, log_0=-1000.0, **targets):
        ...
        ll = pd.DataFrame(np.full((len(smis), len(self._mdl)),
                                  log_0), columns=self._mdl.keys())
        ...
        pred = self.predict(smis).reset_index(
        ...
        for k, (low, up) in self._targets.items():  # k: target; v: (low, up)
            ...
            low_ll = norm.logcdf(low, loc=np.asarray(
                mean), scale=np.asarray(std))

            # calculate up likelihood
            up_ll = norm.logcdf(up, loc=np.asarray(mean),
